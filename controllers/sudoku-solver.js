class SudokuSolver {
  ...
  validate(puzzleString, solver = false) {
    ...
    } else if(solver) {// When validation is called from the solve function. Strings are different according to user stories, but seems redunant (as the error messages say the exact same thing).
      if(!puzzleString){
        return 'g';
      }
    }
